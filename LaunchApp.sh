...
docker run -it --rm --name project_compiler -v "$(pwd)"/Warehouses_Management/Gestion_Almacenes/:/app -w /app maven:latest mvn clean package # -v <source> lo cambias en función de donde se tenga el proyecto

# El .jar generado lo copio en el directorio de app (evitar posibles problemas)
cp "$(pwd)"/Warehouses_Management/Gestion_Almacenes/target/Gestion_Almacenes-1.0-SNAPSHOT.jar "$(pwd)"/app
